...
parser.add_argument('--save_path', type=str, default='./CHKP_FT/', help='')
...
if __name__=='__main__':
    ...
    val_loader, val_num = get_loader(opt.testset_path+'Images/',
                                        opt.testset_path+'Depth/', 
                                        opt.testset_path+'Masks/', 
                                        opt.batchsize, opt.trainsize, num_workers=opt.n_cpu)
    ...
    for epoch in range(cur_epoch, opt.epoch):
        ...
        if (epoch+1) % opt.save_ep == 0:
            torch.save(model.state_dict(), opt.save_path+'lastest_{}_{}_ft.pth'.format(opt.model, opt.dataset))
